...
const addListener = (element: HTMLElement) => (entry: EventAttr) => element.addEventListener(entry[0], entry[1])
const addAttribute = (element: HTMLElement) => (entry: Attr) => element.setAttribute(entry[0], entry[1] as string)

const tag = (tag: string, attrs: Attrs, children?: Children): LMElementNode => {
